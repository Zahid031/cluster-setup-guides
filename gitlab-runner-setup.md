...
- **concurrent**: Number of jobs that can run simultaneously (default: 1)
  - Increase this value to run multiple jobs in parallel
  - Example: `concurrent = 4` allows 4 jobs to run at the same time

- **check_interval**: How often to check GitLab for new jobs in seconds (0 = default 3s)

- **executor**: The execution method (shell, docker, kubernetes, etc.)

- **image**: Default Docker image to use (when using docker executor)

- **privileged**: Enable Docker-in-Docker (set to true if needed)

- **volumes**: Mount volumes inside Docker containers

### Apply Configuration Changes

After editing the configuration file, restart the GitLab Runner service:
